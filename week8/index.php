	...
	<?php }else{?>
		<div class="wrapper bounceInDown animated">
			...
			<div class="container">
				...
				<ul>
					...
					<?php
						if(isset($_SESSION['student_id'])){
							$sql = 'INSERT INTO `signup` (`name`, `student_id`, `opition`) VALUES (:name, :student_id, :opition)';
							$sth = $db->prepare($sql);
							$sth->bindParam(':name', $_POST['name']);
							$sth->bindParam(':student_id', $_POST['student_id']);
							$sth->bindParam(':opition', $_POST['opition']);
							$sth->execute();
				...
				<table>
					<thead>
						<tr><td>學號</td><td>姓名</td><td>想對丁丁說的話</td></tr>
					</thead>
					<tbody>
						<?php
							...
							foreach($list as $data){
								echo '<tr><td>'.$data['student_id'].'</td><td>'.$data['name'].'</td><td>'.$data['opition'].'</td></tr>';
							}
						?>
					</tbody>
				</table>
